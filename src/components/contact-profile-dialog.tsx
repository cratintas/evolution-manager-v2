...
import { formatJid, isGroupJid } from "@/pages/instance/Chat/chat-utils";
...
export function ContactProfileDialog({
  ...
  const name = data?.name?.trim() || fallbackName || formatJid(remoteJid);
  const picture = data?.picture || fallbackPicture;
  const about = typeof data?.status === "string" ? data.status : "";
  const phone = formatJid(data?.wuid || remoteJid);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-sm overflow-hidden p-0 sm:max-w-sm" showCloseButton>
        <div className="flex flex-col items-center bg-primary px-6 pb-8 pt-12 text-primary-foreground">
          ...
          <h2 className="mt-4 text-center text-2xl font-semibold leading-tight">{name}</h2>
          {isFetching && (
            <div className="mt-2">
              <LoadingSpinner />
            </div>
        ...
        <div className="space-y-4 bg-card px-6 py-5 text-card-foreground">
          ...
          {data?.isBusiness ? (
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-medium">{t("contacts.profile.business", { defaultValue: "Conta comercial" })}</p>
